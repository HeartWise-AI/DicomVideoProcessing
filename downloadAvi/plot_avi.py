    ...
    for i, filename in enumerate(sampled_filenames):
        ...
        if filename.endswith(".dcm"):
            ...
            if "video" not in data:
                print(f"Warning: 'video' not found in zarr file {filename}")
                continue
            vid = np.array(data["video"])  # (64, 128, 128)
            if vid.ndim == 3:  # If the video is grayscale
                vid = np.stack([vid] * 3, axis=-1)  # Convert to RGB
            elif vid.ndim == 4 and vid.shape[-1] != 3:
                vid = vid[..., :3]  # Take only the first 3 channels if more than 3

            # Get the middle frame
            middle_frame_index = vid.shape[0] // 2
            frame_rgb = vid[middle_frame_index]
